...
/**
 * Shell for LK Designs — designs only (shop manage + customer browse).
 */
export default async function DesignsAppLayout({ children }: { children: ReactNode }) {
  const locale = await getLocale();
  const session = await getSession();
  const showProfile = session?.role === "SHOP" || session?.role === "CUSTOMER";
    ...
    <div className="partner-app-shell brand-page-bg min-h-dvh w-full min-w-0">
      <header className="partner-app-header">
        <div className="partner-app-header-inner">
          <Link href="/designs" className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
            <BrandLogoMark
              locale={locale}
      ...
      <div className="app-page-shell mx-auto w-full max-w-5xl px-3 pb-6 pt-2 sm:px-4 sm:pb-8">
        {children}
      </div>
    </div>
  );
