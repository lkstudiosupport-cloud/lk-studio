...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { phone, idToken, role, deviceId, latitude, longitude, address, locationLink } =
      parsed.data;
    ...
    const ok = await verifyFirebasePhoneToken(idToken, e164);
    if (!ok) {
    ...
    console.error("Firebase phone login verify error:", err);
