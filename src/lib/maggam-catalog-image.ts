...
function embroideryForTier(index: number, tier: DesignSizeTier): string {
  ...
  if (tier === "SMALL") {
    const y = 580 + (index % 4) * 8;
    return `
      <g opacity="0.96">
        ${templeArch(200, 160, 200, 75, accent)}
        ${sunburst(300, 320, 12, 55 * scale)}
        ${motifByType(type, 100, y - 120, scale, accent)}
        ${motifByType(type + 3, 280, y - 80, scale * 0.95, accent)}
        ${motifByType(type + 7, 400, y - 100, scale * 0.9, accent)}
        ${floralVine(110, y - 30, 90, accent)}
        ${floralVine(360, y - 20, 85, accent)}
        ${peacockFeather(300, y - 55, scale * 0.75, accent)}
        ${coinRow(85, y + 15, 24)}
        ${zariLine(65, y + 32, 535, y + 32, 3.5)}
        ${zariLine(85, 130, 515, 130, 2)}
        ${zariLine(95, 200, 505, 200, 1.5)}
        ${Array.from({ length: 9 }, (_, i) => bead(110 + i * 48, 128, 3)).join("")}
        ${Array.from({ length: 7 }, (_, i) => bead(130 + i * 55, 198, 2.5)).join("")}
      </g>
    `;
  }
...
function labelSvg(catalogNumber: string): string {
  return `
    <rect x="12" y="748" width="210" height="36" rx="8" fill="#000000" opacity="0.55"/>
...
export function parseCatalogCode(
  code: string
): { tier: DesignSizeTier; index: number; catalogNumber: string } | null {
  const normalized = code.replace(/\.jpg$/i, "").trim().toUpperCase();
  const m = normalized.match(/^MAG-(S|M|B)-(\d{4})$/);
  if (!m) return null;

  const tier: DesignSizeTier = m[1] === "S" ? "SMALL" : m[1] === "M" ? "MEDIUM" : "BIG";
  const index = parseInt(m[2]!, 10);
  if (index < 1 || index > 9999) return null;

  return { tier, index, catalogNumber: normalized };
}
