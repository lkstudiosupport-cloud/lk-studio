...
export function ShopDesignsPanel({
  ...
  const stitchedDesigns = useMemo(
    () => designs.filter((d) => d.category === SHOP_UPLOAD_CATEGORY && !d.isCatalog),
    [designs]
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="page-title">{t(locale, "categories.stitched")}</h1>
        <p className="mt-1 text-sm text-zinc-600">{t(locale, "shopStitchedOnlyHint")}</p>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="rounded-full bg-brand-cream px-3 py-1 text-xs font-semibold text-brand-green">
          {stitchedDesigns.length} {t(locale, "collectionItems")}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        ...
        {stitchedDesigns.map((d) => (
          <ShopDesignItem key={d.id} design={d} locale={locale} manageable />
        ))}
      </div>

      {stitchedDesigns.length === 0 && (
