...
export function PhotoSlotsUpload({
  locale,
  max,
  files,
  onFilesChange,
  ...
  /** When set, hidden inputs are rendered for server actions / form posts. */
  fieldPrefix?: string;
  label?: string;
  showCount?: boolean;
  compress?: boolean;
  ...
  function syncFormInputs(next: SlotPreview[]) {
  ...
  const canAddMore = previews.length < max;

  return (
    <div className="space-y-3">
      {label && <p className="text-sm font-semibold text-brand-green">{label}</p>}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {canAddMore && (
          <button
            ...
            className={`flex ${slotSize} shrink-0 flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-brand-green/25 bg-brand-cream/60 text-brand-green transition hover:border-brand-gold hover:bg-brand-cream active:scale-[0.98]`}
            aria-label={t(locale, "addPhoto")}
          >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
              <p className="font-semibold text-brand-green">{t(locale, "addPhoto")}</p>
              <button
                type="button"
                onClick={() => setMenuOpen(false)}
